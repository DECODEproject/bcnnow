...
from apps.backend.data.collectors.pull.OHBCollector3.Config import Config as collectorConfig
collectorCfg = collectorConfig().get()

from config.Config import Config as globalConfig
globalCfg = globalConfig().get()

from apps.backend.data.collectors.pull.OHBCollector3.OHBPayload3 import OHBPayload3
...
class OHBCollector3:
    ...
    def buildRecord(self, item, type):
        ...
        record.setSource(collectorCfg['collectors']['OHB3']['source_name'])
        record.setProvider('')
        record.setPublisher('')
...
if __name__ == "__main__":
    base = collectorCfg['collectors']['OHB3']['base_url']
    resourceIDs = collectorCfg['collectors']['OHB3']['paths']
    OHBCollector3().start(base, resourceIDs)